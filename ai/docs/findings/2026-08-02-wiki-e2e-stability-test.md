...
- **`lint.py`의 따옴표/강조 정규화 수정(2-3 항목)이 실제로 이 문서를 통과시키는지 재측정
  필요** — 코드 레벨로는 검증했지만 실기동 확인은 아직. 재측정 전에 반드시 사용자와 먼저
  상의할 것 (같은 문서에 이미 $11+ 소진, [[confirm-before-rerunning-expensive-live-tests]]).
- (참고, 우선순위 낮음) `lint.py`의 "error 없으면 끝" 종료 신호(2-2 항목)의 실제 효과는
  여전히 미검증 상태 — 따옴표 정규화로 애초에 warn 자체가 안 나면 이 문제는 자연히 줄어들
  수 있음.
