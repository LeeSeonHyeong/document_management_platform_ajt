...
- **2026-08-01 갱신**: 챗봇을 같은 방향으로 먼저 끝내고(S15P11B106-170, 계약 1.8.0) 그 실측·실연동
  으로 이 설계의 세 곳을 확정했다 — §3.4.1(수정 상한) · §3.7(도구 전송 수단) · §8.2 미결 3번(선택
  삭제). 근거는 `2026-07-31-agent-endpoint-merge-design.md` 와 `experiments/INDEX.md`
  「위키 1단계 선택의 천장」이다
...
#### 3.4.1 수정 페이지 수에 상한을 건다 (2026-08-01 추가)

창구로 바꾸면 에이전트가 **범위 전체**를 읽을 수 있게 된다. 지금은 실려 온 5장이 곧 상한이라
폭주가 구조적으로 막혀 있는데, 그 울타리가 사라진다.

**한 작업이 고칠 수 있는 서로 다른 페이지를 10장으로 묶는다.** 실측이 근거다 — 문서 1건이
건드린 페이지가 **중앙 3~4장 · 최대 7장**이다 (`2026-07-27-opus46-12docs` 12건 ·
`2026-07-29-sonnet46-12docs` 12건). 10 은 실측 최대의 1.4배다.

상한은 **서로 다른 페이지 수에만** 건다. 같은 페이지를 여러 번 고치는 것은 막지 않는다 —
실측에서 `edit` 이 문서당 19회인데 그것이 3~4장에 반복해 떨어진다 (쓰기의 92%가 「고치기」다).
그쪽은 턴 상한이 이미 묶는다.

넘으면 **도구가 거절하고 실행은 계속된다.** 「상한에 닿았으니 지금까지 것으로 마무리하고
lint 를 부르라」를 문장으로 돌려준다 — 챗봇의 조회 실패 처리와 같은 방식이다. 실행을 죽이면
그때까지 만든 것이 통째로 사라진다.
...
### 3.7 도구를 어떻게 붙이나 — MCP 는 사람용으로 남긴다 (2026-08-01 추가)

지금은 자동 실행도 stdio MCP 하위 프로세스를 통한다. 그것이 S15P11B106-152 의 미결 둘을
만든다 — **허가값을 자식 프로세스에 넘길 수단**(CLI 인자는 `/proc/<pid>/cmdline` 이
world-readable 이라 불가)과 **프로세스 밖 중단 신호**다.

**구현과 전송을 나눈다.** 도구 구현은 하나(`wiki_mcp/tools/*`)로 두고 전송만 둘로 한다.

| 소비자 | 전송 | 이유 |
| --- | --- | --- |
| 사람 (터미널 Claude Code) | stdio MCP 서버 (`local_server.py`) | 사람이 붙여 쓰는 것이 이 서버의 존재 이유다. 유지한다 |
| AI 서버의 자동 실행 | **in-process** (`agent_runtime.tools.AgentTool`) | 하위 프로세스가 없으니 위 미결 둘이 생기지 않는다 |

`tools/__init__.py` 의 `register(mcp, get_scope_key, fs_factory)` 가 이미 구현과 MCP 를 갈라
놓았다. 그 자리에 in-process 어댑터를 하나 더 붙이는 일이다.

**근거는 챗봇이다.** S15P11B106-170 이 도구 다섯 개를 런타임에 직접 붙여 실기동·Spring
실연동까지 통과했다 (2026-07-31). 그 경로에는 152 의 미결이 애초에 없었다.

**잃는 것을 적어 둔다.** 프로세스 경계가 주던 「프로세스 1개 = 스코프 1개 = 작업 1개」 격리가
in-process 에서는 **규율로 내려온다** — 도구를 그 스코프·그 작업에 묶어 만드는 것을 한 곳에서만
하고 그 한 곳을 테스트로 고정한다. 쓰기가 있는 경로라 구조적 보장이 규율로 바뀌는 것은 손실이다.
사람용 MCP 는 프로세스 격리를 그대로 유지한다.
...
| ~~3~~ | ~~`wiki-context-selections` 삭제 여부~~ | **결정됨 (2026-08-01): 삭제한다.** 삭제·교체의 역조회는 `GET /internal/v1/wiki-spaces/{scopeKey}/relations` 가 스코프 전체의 `documentRefs` 를 한 번에 주므로 창구로 해결된다. 우리가 그것을 걸러 `wikis_citing(documentId)` 도구로 준다 — 에이전트가 목록을 직접 해석하면 빠뜨릴 여지가 생긴다 |
...
### 9.2 남은 순서 (2026-08-01 갱신)

위 목록의 6·7 이 부분적으로 끝났다 — Spring 질의 API 8개는 develop 에 있고(S15P11B106-150),
AI 쪽 `WikiQueryClient`·`FederatedVaultFS` 도 있다. 창구 경로가 fail-closed 로 막혀
한 번도 실경로로 돌지 않았는데, **2026-08-01 에 그 셋을 끝내고 Spring 실연동까지 통과했다**
(아래 인수 조건).

| 순서 | 무엇 | 티켓 | 상태 |
| --- | --- | --- | --- |
| 1 | 본문 적재 때 참조 동기화를 그 주소로 좁힌다 | S15P11B106-151 | **끝** (2026-08-01) |
| 2 | 라이브 본문을 청크 색인하지 않는다 | S15P11B106-154 | **끝** (2026-08-01) |
| 3 | 자동 실행을 in-process 도구로 배선하고 fail-closed 를 걷는다 | S15P11B106-152 (축소) | **끝** (2026-08-01) |
| 4 | `wiki-context-selections` 삭제 · 계약 1.9.0 · `wikis_citing` 도구 | 신규 | 남음 |

**4 의 백엔드 몫은 넘긴다** — `WikiTransformationService`·`DocumentParseWorker` 가 선택 호출을
그만두고 `selectedWikis` 를 안 싣는 일이다. 둘 다 도메인 서비스라 AI 담당 범위 밖이다
(`ai/CLAUDE.md`). 계약을 먼저 머지하고 동시 진행한다 — S15P11B106-169/170 이 그 방식으로 됐다.
**엔드포인트를 먼저 지우면 Spring 이 404 를 받는다** — 계약·백엔드가 함께 가야 한다.

#### 인수 조건 — Spring 실연동 (2026-08-01 통과)

문서 업로드부터 위키 반영까지 한 건이 끝까지 갔다. 단위 테스트와 실기동 스모크만으로는
부족하다는 것을 챗봇에서 확인했었다 — 그 둘을 다 통과한 코드가 서버 기동 후 두 번째
요청부터 전부 실패하고 있었다 (2026-07-31).

구성: Spring `local` 프로파일(H2) + AI 서버 `deepagents`/`openai:gpt-5.4` +
`BACKEND_BASE_URL=http://localhost:8080`. 관리자 업로드 → `ai-jobs/{id}/start`.

로그가 확인해 준 것:

| 무엇 | 관측 |
| --- | --- |
| 창구 경로가 실제로 열렸나 (152) | 하이드레이션 3회 200 — `categories`·`wiki-pages`·`index` |
| fan-out 이 없나 (151) | 카탈로그 2장에 본문 조회 2회. 링크를 따라 더 당기지 않았다 |
| 결과 | 위키 1건 갱신(각주 7·목차 링크 1), `wiki-transformations` 200, Spring 이 DB 반영 |

**실연동이 잡은 결함 둘.**

1. `call_tool` 반환 모양이 도구마다 다르다. `structured_output=False` 인 `read` 만
   튜플이 아니라 목록을 준다 — 튜플로 단정해 풀다 `ValueError` 로 죽었다. 고쳤고
   테스트를 붙였다 (`tests/agent_runtime/test_wiki_tools.py`)
2. **백엔드**: `WikiCapabilityService.require` 가 `remove` 후 `put` 으로 되돌린다.
   에이전트가 한 턴에 조회를 여러 개 동시에 부르면 그 사이에 들어온 요청이 `null` 을
   받아 만료로 오판한다. 실측에서 4건 중 3건이 404. S15P11B106-172 로 넘겼다 —
   **flaky 라 고쳐지기 전까지 간헐 실패가 남는다**

#### MySQL 로 다시 (2026-08-01) — 검색 경로까지 통과

H2 에서는 `wiki-search` 가 500 이었다. `WikiSearchChunkRepository` 의
`match(content) against(...)` 이 MySQL 전용이라 `Function "match" not found` 가 났다.
MySQL 8.0.46 인스턴스를 따로 띄워(포트 3307 · `ngram_token_size=2` · `erd.sql` 적용)
다시 돌렸다. `ddl-auto=validate` 가 통과했다 — **엔티티가 ERD 와 맞는다.**

| 창구 | 결과 |
| --- | --- |
| `wiki-search` | **200 × 14** (H2 에서는 전부 500), 404 × 1 은 허가 경합 |
| `wikis/{id}/content`·`relations` | 200 × 27, 404 × 2 (같은 경합) |
| `wiki-pages`·`wiki-spaces/*` | 200 × 9 |

한국어 ngram 전문 검색이 실제로 맞는다 (색인된 청크 4건에 직접 질의):
...
에이전트도 창구 검색으로 기존 위키를 찾았다 — 두 번째 문서에서 `경조 휴가`·
`연차 촉진`·`미사용 연차`·`반차` 로 질의해 전부 200 을 받았다.

**MySQL 에서만 드러난 백엔드 결함 하나 더.** 기존 Wiki 를 갱신하면 검색 청크 재색인이
`Duplicate entry '1-0' for key 'wiki_search_chunk.uk_wiki_search_chunk'` 로 죽는다.
`WikiSearchIndexer.replace` 가 파생 삭제 뒤 바로 `saveAll` 하는데 Hibernate 가 flush 때
INSERT 를 DELETE 보다 먼저 낸다. H2 는 기동마다 `create-drop` 이라 갱신 대상에 청크가
없어 안 걸렸다. S15P11B106-173 으로 넘겼다.

**남은 제약**: 이 MySQL 은 8.0.46 이고 ERD 기준은 8.4 다. `9.1 MySQL 실측` 은 8.4.11
컨테이너에서 쟀다 — 전문 검색 동작은 같았지만 판본이 다르다는 것은 적어 둔다.
