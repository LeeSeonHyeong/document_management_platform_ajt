...
def ingest_instruction(document_address: str, scope_key: str) -> str:
    ...
    return (
        ...
        f"마지막 답변은 이 작업의 기록이다. 관리자가 나중에 「AI 작업 요약」 목록에서 훑어보며 "
        f"어떤 문서로 무엇이 바뀌었는지 확인하는 용도다. **읽는 사람은 개발자가 아닌 비개발자 "
        f"관리자다** — 만든 파일 이름이나 시스템 용어(scope·frontmatter·lint 등)를 쓰지 않는다. "
        f"**「오류·에러·통과·검증·확인 완료」 같은 점검 결과를 나타내는 말을 한 마디도 쓰지 "
        f"않는다.** 그건 관리자가 알 필요 없는 내부 확인 절차이고, 관리자는 그 말이 무엇을 "
        f"뜻하는지도 모른다. 점검이 끝났다는 말로 시작하거나 끝맺지 않는다.\n\n"
        f"**반드시 한국어로 쓴다.** 관리자는 한국어만 읽는다.\n\n"
        f"**아래 양식을 그대로 지킨다** — 한 줄 요약, 표 하나(다룬 페이지마다 한 행), 그다음 "
        f"「반영하지 않은 내용」 한 줄이다. 페이지는 파일 이름이 아니라 제목으로 적고, "
        f"「처리」는 새로 만들었는지 고쳤는지다. 「근거」는 그 페이지에 단 각주 개수다.\n\n"
        f"```\n"
        f"**2장**을 다뤘습니다 (새로 1장, 고침 1장).\n\n"
        f"| 페이지 | 처리 | 카테고리 | 주요 내용 | 근거 |\n"
        f"| --- | --- | --- | --- | --- |\n"
        f"| 연차 휴가 산정 | 새로 만듦 | 근무 정책 | 입사일 기준 산정, 이월 규칙 | 12곳 |\n"
        f"| 휴가 정책 개요 | 고침 | 근무 정책 | 연차 항목을 최신 규정에 맞게 정리 | 5곳 |\n\n"
        f"- 반영하지 않은 내용: 없습니다\n"
        f"```\n\n"
        f"**「주요 내용」에 무엇이 바뀌었는지 구체적으로 적는다.** 「내용을 보강했습니다」 "
        f"같은 말은 아무것도 알려주지 않는다 — 어떤 항목이 어떻게 바뀌었는지 쓴다 "
        f"(예: 「식비 1일 3만원 신설, 4월 1일 시행」).\n\n"
        f"**「반영하지 않은 내용」을 정직하게 쓴다.** 원본문서에 있었지만 위키에 넣지 않은 것을 "
        f"적는다 — 근거가 약해 뺐거나, 이 범위와 무관하거나, 이미 다른 페이지에 있어 생략한 것. "
        f"관리자가 그걸 보고 빠진 게 없는지 판단한다. 정말 다 넣었으면 「없습니다」라고 쓴다. "
        f"**빠뜨린 것을 숨기지 않는다.** 그 줄 자체를 빼지 않는다.\n\n"
        f"나쁜 예: \"pages/a3f2.md edit, lint 0 errors.\"\n"
        f"나쁜 예: \"작업을 완료했습니다.\"\n"
        f"나쁜 예: \"error 0건, lint 통과 확인 완료. 연차 신청 규정을 반영했습니다.\"\n"
        f"나쁜 예: \"오류 없이 통과했습니다. 연차 신청 규정을 반영했습니다.\"\n"
        f"나쁜 예: 표 없이 문장만 길게 쓴 것 — 목록 화면에서 훑어볼 수 없다."
    )
...
def reconcile_instruction(document_address: str, scope_key: str, change_type: str,
    ...
    if change_type == "document_replaced":
        ...
        # 교체 분기와 같은 이유로 **작업 공간의 실제 상태를 적어 준다.** 삭제 재조정도
        # 지울 문서의 옛 본문을 얹는다 (`routers/wiki.py` 의 `load_source`) — 그래야 각주가
        # 풀려 backlink 가 잡힌다. 그 사실을 적지 않으면 「삭제됐다」는 지시와 「read 하면
        # 나온다」는 관측이 어긋나, 에이전트가 무엇이 잘못됐는지 찾으려 검색을 반복하다
        # 턴 상한에서 죽는다 (2026-08-06 job 35, `search` 32회 · 쓰기 0건).
        # `tests/mcp/test_removal_self_contradiction.py` 가 그 상태를 실측으로 고정한다.
        what = (f"원본문서 `{document_address}`가 **삭제**됐다. **그 주소를 `read` 하면 "
                f"내용이 나오지만 그것은 무엇을 걷어낼지 확인하라고 남겨 둔 참고용 사본이다** "
                f"— 라이브 위키에는 더 이상 없다. 문서가 아직 있다는 뜻이 아니므로 그것을 "
                f"확인하려 다시 찾지 않는다. 이 문서를 근거로 쓴 문단과 각주를 걷어낸다. "
                f"근거가 전부 사라진 페이지는 `delete` 한다.")

    return (
        f"`guide` 도구를 먼저 불러 작업 방식을 확인한다.\n\n"
        f"범위 `{scope_key}`에서 {what}\n\n"
        f"고칠 곳은 다음과 같다:\n{listing}\n\n"
        f"**이 목록이 전부다.** 참조 그래프가 각주 단위로 이미 알고 있으므로 이 밖에서 "
        f"고칠 곳을 찾지 않는다. 다른 문서를 근거로 한 문단은 건드리지 않는다. 문단을 "
        f"지우면 남은 각주 번호가 어긋나지 않게 정리한다. 마지막에 `lint`를 부른다.\n\n"
        f"`lint`가 `error`를 내면 고치고 다시 부른다. 다만 **`lint`로 이 작업의 완료를 "
        f"판단하지 않는다** — 참고용 사본이 올라와 있어 걷어내야 할 각주가 미해결로 잡히지 "
        f"않는다. 위 목록을 다 처리했으면 끝난 것이다."
    )
...
def edit_instruction(wiki_address: str, scope_key: str, instruction: str,
    ...
    return (
        ...
        f"각주를 유지하고, 본문이 바뀌면 frontmatter의 `description`도 맞춘다.\n\n"
        ...
        f"요청한 내용을 어떻게 고쳤는지 2~3문장으로 쓴다. **「오류·에러·통과·검증·확인 완료」 "
        f"같은 점검 결과를 나타내는 말을 답변에 한 마디도 쓰지 않는다.** 그건 관리자가 알 필요 "
        f"없는 내부 확인 절차이고, 관리자는 그 말이 무엇을 뜻하는지도 모른다. 답변의 첫 단어부터 "
        f"곧바로 반영된 내용을 설명한다 — 점검이 끝났다는 말로 시작하거나 끝맺지 않는다.\n\n"
        f"**반드시 한국어로 쓴다.** 관리자는 한국어로 요청했고 한국어만 읽는다. 페이지를 "
        f"가리킬 때는 파일 이름이 아니라 그 페이지의 제목으로 부른다.\n\n"
        f"나쁜 예: \"error 0건, lint 통과 확인 완료. 연차 신청 규정을 반영했습니다.\"\n"
        f"나쁜 예: \"오류 없이 통과했습니다. 연차 신청 규정을 반영했습니다.\"\n"
        f"나쁜 예: \"Everything checks out fine — pages/a3f2.md already reflects the "
        f"current content.\" (영어로 쓴 것도, 점검 결과로 시작한 것도, 파일 이름을 쓴 것도 "
        f"잘못이다)\n"
        f"좋은 예: \"「출장비 정산 안내」 페이지 맨 위에 출장 기간 규정을 한 줄로 요약해 "
        f"넣었습니다. 나머지 내용과 근거 표시는 그대로 두었습니다.\""
    )
