...
def ingest_instruction(document_address: str, scope_key: str) -> str:
    ...
    return (
        ...
        f"마지막 답변으로 이 작업을 관리자에게 보고하는 문장을 쓴다. **읽는 사람은 개발자가 "
        f"아닌 비개발자 관리자다** — 만든 파일 이름이나 시스템 용어(scope·frontmatter·lint 등)를 "
        f"쓰지 않고, 이 문서로 위키에 무엇이 새로 반영되거나 바뀌었는지 2~3문장으로 쓴다. "
        f"**「오류·에러·통과·검증·확인 완료」 같은 점검 결과를 나타내는 말을 답변에 한 마디도 "
        f"쓰지 않는다.** 그건 관리자가 알 필요 없는 내부 확인 절차이고, 관리자는 그 말이 무엇을 "
        f"뜻하는지도 모른다. 답변의 첫 단어부터 곧바로 반영된 내용을 설명한다 — 점검이 끝났다는 "
        f"말로 시작하거나 끝맺지 않는다.\n\n"
        f"좋은 예: \"연차 신청 규정 문서를 반영해 '연차 휴가 산정' 페이지에 이월 규칙을 "
        f"추가하고, 같은 내용을 다루던 '휴가 정책 개요' 페이지도 최신 규정에 맞게 정리했습니다.\"\n"
