...
def ingest_instruction(document_address: str, scope_key: str) -> str:
    ...
    return (
        ...
        f"보충 항목 네 줄이다. 페이지는 파일 이름이 아니라 제목으로 적고, 「처리」는 새로 "
        f"만들었는지 고쳤는지다. 「근거」는 그 페이지에 단 각주 개수다.\n\n"
        f"```\n"
        ...
        f"- 태그: 연차, 휴가, 규정, 산정\n"
        f"- 시각 자료: 연차 산정 흐름도, 이월 조건 표\n"
        f"- 상호 링크: 「휴가 정책 개요」↔「연차 휴가 산정」\n"
        f"- 반영하지 않은 내용: 없습니다\n"
        f"```\n\n"
        ...
        f"**빠뜨린 것을 숨기지 않는다.** 시각 자료나 상호 링크를 만들지 않았으면 그 줄에 "
        f"「없습니다」라고 쓴다 — 줄 자체를 빼지 않는다.\n\n"
