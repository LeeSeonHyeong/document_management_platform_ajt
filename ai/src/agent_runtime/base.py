...
def ingest_instruction(document_address: str, scope_key: str) -> str:
    ...
    return (
        ...
        f"나쁜 예: \"pages/a3f2.md edit, index.md 갱신, lint 0 errors.\"\n"
        f"나쁜 예: \"작업을 완료했습니다.\"\n"
...
def edit_instruction(wiki_address: str, scope_key: str, instruction: str,
    ...
    return (
        ...
        f"**반드시 한국어로 쓴다.** 관리자는 한국어로 요청했고 한국어만 읽는다. 목차를 가리킬 "
        f"때는 파일 이름이 아니라 「목차」라고, 페이지를 가리킬 때는 그 페이지의 제목으로 부른다.\n\n"
        f"나쁜 예: \"error 0건, lint 통과 확인 완료. 연차 신청 규정을 반영했습니다.\"\n"
        f"나쁜 예: \"오류 없이 통과했습니다. 연차 신청 규정을 반영했습니다.\"\n"
        f"나쁜 예: \"Everything checks out fine — index.md already reflects the current "
        f"content.\" (영어로 쓴 것도, 점검 결과로 시작한 것도, 파일 이름을 쓴 것도 잘못이다)\n"
        f"좋은 예: \"「출장비 정산 안내」 페이지 맨 위에 출장 기간 규정을 한 줄로 요약해 "
        f"넣었습니다. 나머지 내용과 근거 표시는 그대로 두었고, 목차의 소개 문구도 여전히 맞아 "
        f"손대지 않았습니다.\""
    )
