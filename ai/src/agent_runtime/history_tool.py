...
# 응답 상한. **한국어는 ~1자=1토큰이다** (compaction 설계 문서의 count_tokens 실측
# 0.85~1.02) — 처음 값 40,000자는 영어(~4자=1토큰) 감각이었고, 이력 읽기 한 번이
# ~40K 토큰을 되돌려 GMS 42K 벽으로 직행하는 문맥 폭탄이었다 (2026-08-06, 문서 48
# 두 번째 실패: 압축 직후 11K → 이력 읽기 → 34K → 또 압축, 한 실행에 압축 8회).
#
# 근거는 압축 트리거의 여유 설계다: 트리거 34K 는 「1턴 최대 증가 ~4.6K 토큰」을
# 전제로 잡혔다 (42K 벽 − 34K − 여유). 도구 결과도 그 계약 안에 있어야 하므로
# 4,000자(≈ 한국어 4K 토큰)다. 세부가 더 필요한 경우를 위한 값이 아니다 — 진행
# 상태의 정본은 압축 요약(`_WIKI_STATE_SUMMARY_ADDENDUM`)이고 이 도구는 보조다.
_MAX_CHARS = 4_000
...
def history_read_tool(backend) -> AgentTool:
    ...
    return AgentTool(
        ...
        description=(
            ...
            f"**진행 상태의 정본은 요약이다** — 요약에 남은 일이 적혀 있으면 그것을 "
            f"따르고, 이 도구는 요약에 없는 세부가 꼭 필요할 때만 부른다. 최근 부분만 "
            f"돌려주며, 같은 파일을 반복해 읽어도 새 정보가 나오지 않는다. "
