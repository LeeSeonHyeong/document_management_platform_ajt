...
- **원문 문장 안에 마크다운 링크(`[글자](주소)`)가 있으면 그 문법까지 그대로 옮긴다.**
  링크 부분만 풀어서 "글자"로 적으면 원문과 문자 그대로 다른 문장이 돼 `citation-quote-not-found`가
  난다 — 하네스 실측(2026-08-02)에서 가장 잦았던 패턴이다. 이것도 `warn`이라 반복되면 넘어간다
...
**원본문서 안의 링크(`/handbook/...`, `https://...` 등)는 그대로 옮기지 않는다.** 그건 원본이
속한 외부 사이트의 주소지 이 위키의 페이지가 아니다 — 그대로 두면 `lint`가
`dangling-link`로 잡는다. 같은 내용을 다루는 페이지가 이 위키 안에 있으면 그 페이지 주소로
바꿔 링크하고, 없으면 링크를 빼고 일반 텍스트로 쓴다.
