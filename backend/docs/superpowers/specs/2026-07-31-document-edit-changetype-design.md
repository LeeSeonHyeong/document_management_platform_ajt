...
`DocumentManagementService:181`의 `TODO(계약)`이 여기서 해소된다. 재처리 범위는 (a) 이 문서
1건 증분으로 확정한다.

### 5.4 문서 삭제 — 확정 필요

`DELETE /api/v1/documents/{documentId}`는 지금 `reprocessScope`로 남은 문서 전체를 다시
`document_added`로 밀어넣는다. 위 정책과 같은 뿌리의 문제이므로 **`document_removed` 1건으로
바꾸는 것이 일관된다.** `DocumentManagementService:286`의 `TODO(위키)`(고아 Wiki 정리)도 같이
해소된다.

다만 이번 결정 범위에 명시적으로 포함되지 않았다. 삭제는 파일을 지우므로 옛 parsed 본문을
지우기 전에 읽어야 한다는 제약이 §3과 동일하다. **팀 확정 후 진행한다.**
...
두 건 모두 프론트엔드 담당 동의가 필요하다. 합의 전에는 구현을 시작하지 않는다.

## 7. 작업 순서

1. **계약 변경 합의** (§6) — 프론트엔드 담당과 확정. 미합의 시 나머지가 막힌다.
2. **옛 parsed 본문 보존** (§3) — 계약과 무관하게 독립적으로 선행 가능.
3. **재처리 계획 전달** (§4) — 워커·`WikiTransformationService` 시그니처 일반화.
4. **경로별 배선** (§5.1~5.3).
5. **문서 삭제** (§5.4) — 별도 확정 후.
