...
JWT 액세스 토큰을 URL, 쿼리 파라미터 또는 요청 본문으로 전달하지 않습니다. 이메일 비밀번호 재설정 링크의 단기 서명 토큰은 8.2절의 예외 규칙을 따릅니다.
...
등록된 활성 계정이면 HTTPS 기반 재설정 링크를 이메일로 전송합니다. 링크의 서명 토큰은 다음 규칙을 따릅니다.

- 유효시간은 30분입니다.
- 토큰 용도를 비밀번호 재설정으로 제한합니다.
- 사용자 ID와 현재 비밀번호 해시에 연결해 서명합니다.
- 비밀번호가 변경되면 기존 토큰은 자동으로 무효화됩니다.
- 토큰 원문 또는 토큰 해시를 DB에 저장하지 않습니다.
- 재설정 요청에는 IP와 이메일 기준 요청 횟수 제한을 적용하고 초과 시 `429 Too Many Requests`를 반환합니다.

프론트엔드는 이메일 링크에서 받은 토큰과 새 비밀번호를 `POST /api/v1/auth/password-resets` 요청 본문으로 전달합니다. 유효한 토큰으로 비밀번호를 변경하면 `204 No Content`를 반환하며 자동 로그인하지 않습니다.

유효하지 않거나 만료된 토큰은 `400 Bad Request`와 `INVALID_OR_EXPIRED_RESET_TOKEN`을 반환합니다. 비밀번호 재설정 요청과 응답에는 이메일 등록 여부, 사용자 ID와 계정 상태를 노출하지 않습니다.
