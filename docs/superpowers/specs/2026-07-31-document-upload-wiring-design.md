...
- 공통 axios 클라이언트의 JSON 기본 헤더가 `FormData`를 JSON으로 변환하지 않도록
  문서 업로드와 문서 파일 교체 요청은 기존 문의 첨부 API와 동일하게
  `multipart/form-data`를 명시한다.
- 문서 카드의 파일 선택 버튼과 드롭존은 기존 사용자 동작을 유지한다.
  카드에서 선택하거나 드롭한 파일은 서버에 즉시 보내지 않고 업로드 모달의
  초기 파일 목록으로 전달한다.
- 업로드 모달에서 공개 범위와 카테고리를 선택하고 업로드를 확정한 시점에만
  `POST /api/v1/documents`를 호출한다.
- 모달 내부의 파일 선택과 드래그앤드롭도 계속 지원하며, 카드에서 전달된 파일에
  추가 파일을 합칠 수 있다.
